...
if __name__ == "__main__":
    ...
    DB.setCredentials(
        symmetric_key=os.getenv(f'PH_VARIANTX_{Env}_DB_SYMMETRIC_KEY').encode(),
        host=os.getenv(f'PH_VARIANTX_{Env}_DB_HOST').encode(),
        port=os.getenv(f'PH_VARIANTX_{Env}_DB_PORT').encode(),
        user=os.getenv(f'PH_VARIANTX_{Env}_DB_USER').encode(),
        password=os.getenv(f'PH_VARIANTX_{Env}_DB_PASSWORD').encode(),
        database=os.getenv(f'PH_VARIANTX_{Env}_DB_DATABASE').encode()
    )
